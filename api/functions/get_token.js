...
exports.handler = async function (event) {
  ...
  let { email, hash } = event.queryStringParameters
  email = decodeURIComponent(email)
  hash = decodeURIComponent(hash)

  // Uses the given email and has to get a TOKEN.user_id
  const sql_check_user = `SELECT USER.user_id FROM USER WHERE email='${email}' AND hash='${hash}' ;`
  ...
  const sql_insert_token = `INSERT into TOKEN (TOKEN.token, TOKEN.user_id) VALUES ( '${token}', ( SELECT USER.user_id FROM USER WHERE email='${email}' AND hash='${hash}' ) ) ;`
